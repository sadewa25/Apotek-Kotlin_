...
class AdapterRecyclerLokasiBarang(
    val context: Context?, var dataItems: ArrayList<ResultItem?>?,
    private val listener: (ResultItem?) -> Unit):
    RecyclerView.Adapter<AdapterRecyclerLokasiBarang.itemHolder>(), AnkoLogger {
    ...
    override fun onBindViewHolder(holder: AdapterRecyclerLokasiBarang.itemHolder, position: Int) {
        if (context != null) {
