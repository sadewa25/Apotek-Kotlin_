...
class AdapterRecyclerKategoriBarang(
    val context: Context?, var dataItems: ArrayList<ResultItem?>?,
    private val listener: (ResultItem?) -> Unit):
    RecyclerView.Adapter<AdapterRecyclerKategoriBarang.itemHolder>(), AnkoLogger {
    ...
    override fun onBindViewHolder(holder: AdapterRecyclerKategoriBarang.itemHolder, position: Int) {
        if (context != null) {
